...
const InformacionUsuario = ({ route }) => {
  ...
  const [opcionVisualizacion, setOpcionVisualizacion] = useState([
    {
      id: 1,
      label: "Tactil",
      selected: alumno.opciones_visualizacion?.includes("Tactil"),
    },
    {
      id: 2,
      label: "Lector de pantalla",
      selected: alumno.opciones_visualizacion?.includes("Lector de pantalla"),
    },
    {
      id: 3,
      label: "Navegación con el teclado",
      selected: alumno.opciones_visualizacion?.includes(
        "Navegación con el teclado"
      ),
    },
    {
      id: 4,
      label: "Lector de texto",
      selected: alumno.opciones_visualizacion?.includes("Lector de texto"),
    },
  ]);
    ...
    [
      {
        id: 1,
        label: "Texto",
        selected: alumno.preferencias_accesibilidad?.includes("Texto"),
      },
      {
        id: 2,
        label: "Pictogramas",
        selected: alumno.preferencias_accesibilidad?.includes("Pictogramas"),
      },
      {
        id: 3,
        label: "Audio",
        selected: alumno.preferencias_accesibilidad?.includes("Audio"),
      },
      {
        id: 4,
        label: "Video",
        selected: alumno.preferencias_accesibilidad?.includes("Video"),
      },
    ]
  );
  ...
  const handleAceptarPress = async () => {
    ...
    if (fotoCambiada) {
      ...
      urlFotoPerfil = await postImagen({
        fotoUri: fotoPerfil,
    ...
    const datosAlumno = {
      ...
      pref_contenido: selectedPreference,
      rol: "ESTUDIANTE",
      id: alumno.id,
    };
  ...
  return (
    <Layaout>
      <ScrollView>
          ...
          <View style={styles.sliderContainer}>
            <View>
              ...
              <View style={styles.sliderContainer}>
                ...
                />
                <Text
                  style={{ fontSize: fontSize, marginBottom: 10, margin: 10 }}
                >
                  Tamaño
                </Text>
              </View>
        ...
        <View style={styles.optionContainer}>
          <Text style={styles.label}>Preferencias de Contenido:</Text>
          <View style={styles.contentPreferenceContainer}>
            {["AUDIO", "VIDEO", "TEXTO", "PICTOGRAMAS"].map((option) => (
              <TouchableOpacity
                key={option}
                style={[
                  styles.contentPreferenceButton,
                  selectedPreference === option &&
                    styles.contentPreferenceSelected,
                ]}
                onPress={() => setSelectedPreference(option)}
              >
                <Text
                  style={[
                    styles.contentPreferenceText,
                    selectedPreference === option &&
                      styles.contentPreferenceTextSelected,
                  ]}
                >
                  {option}
                </Text>
...
const styles = StyleSheet.create({
  ...
  contentPreferenceButton: {
    ...
    borderRadius: 8,
    marginBottom: 10,
    width: "48%", // Ajusta para mantener dos botones por fila
    alignItems: "center",
    justifyContent: "center",
  },
  contentPreferenceSelected: {
